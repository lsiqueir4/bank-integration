    ...
    def proccess_webhook(self):
        try:
            ...
            webhook_type_enumerator = (
                f"{webhook_subscription}_{webhook_event.get('log')}"
            )
            webhook_type = self.webhook_repository.get_webhook_type(
                webhook_type_enumerator
            )
            if not webhook_type:
            ...
            if failure_reason:
                ...
                webhook = self.webhook_repository.update_webhook(
            ...
            print("ERRO:", e)
            print("ORIGINAL:", getattr(e, "orig", None))
            traceback.print_exc()
            raise
